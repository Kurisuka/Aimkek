    ...
    public static class EvadeManager
    {
        ...
        public static void Attach(Menu mainMenu)
        {
            ...
            EvadeSpellMenu = new Menu("evadeSpells", "Spell Settings");
            {
                ...
                {
                    var name = spell.Slot;
                    EvadeSpellMenu.Add(new MenuSeperator("VladimirEvade_" + name, "Vladimir " + name + " Settings"));
                    EvadeSpellMenu.Add(new MenuSlider("DangerLevel_" + name, "Danger level", spell.DangerLevel, 1, 5));
                    EvadeSpellMenu.Add(new MenuBool("Tower" + name, "Don't use Under Tower", false));
        ...
        private static void TryToEvade(List<Skillshot> HitBy, Vector2 Pos)
        {
            ...
            {
                if (evadeSpell.Tower &&
                    ObjectManager.GetLocalPlayer().ServerPosition.PointUnderEnemyTurret())
                {
                    return;
                }
                ...
                {
                    //if (evadeSpell.Slot == SpellSlot.Q)
                    //{
                    //    var points = GetEvadePoints(evadeSpell.Speed, evadeSpell.Delay);

                    //    points.RemoveAll(
                    //        item => item.Distance(ObjectManager.GetLocalPlayer().ServerPosition) > evadeSpell.Range);

                    //    for (var i = 0; i < points.Count; i++)
                    //    {
                    //        points[i] = ObjectManager.GetLocalPlayer().ServerPosition.To2D().Extend(points[i], evadeSpell.Range);
                    //    }
                    ...
                    //    if (points.Count > 0)
                    //    {
                    //        ObjectManager.GetLocalPlayer().SpellBook.CastSpell(evadeSpell.Slot, Pos.Closest(points).To3D());
                    //    }
                    //}
                                        ...
                                        .Any(
                                            x =>
                                                ObjectManager.GetLocalPlayer().SpellBook.CastSpell(SpellSlot.W, 
                                                    ObjectManager.GetLocalPlayer().ServerPosition.Extend(x.Start.To3D(), 300))))
                                {
                                    return;
                                }
                            }
