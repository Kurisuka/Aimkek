...
namespace Lil_BloodPump.SpellBlocking
{
    #region
    ...
    public class Skillshot
    {
            ...
            get
            {
                ...
                if (IsGlobal)
                {
                    return GlobalGetMissilePosition(0) +
                           ...
                           (0.5f + SpellData.Radius * 2 / ObjectManager.GetLocalPlayer().MoveSpeed);
                }
        ...
        {
            return EvadeManager.SkillShotMenu["Evade" + SpellData.ChampionName.ToLower()][name + SpellData.MenuItemName].As<MenuSlider>().Value;
        }

        public bool GetBool(string name)
        {
            return EvadeManager.SkillShotMenu["Evade" + SpellData.ChampionName.ToLower()][name + SpellData.MenuItemName].As<MenuBool>().Value;
        }
        ...
        public SafePathResult IsSafePath(List<Vector2> path, int timeOffset, int speed = -1, int delay = 0, Obj_AI_Base unit = null)
        {
            var Distance = 0f;
            ...
            if (unit == null)
            {
                unit = ObjectManager.GetLocalPlayer();
            }
            ...
            {
                if (IsSafe(ObjectManager.GetLocalPlayer().ServerPosition.To2D()))
                {
                        ...
                        if (i == allIntersections.Count - 1)
                        {
                            ...
                            return
                                new SafePathResult(
                                    ...
                                    ObjectManager.GetLocalPlayer().MoveSpeed < SpellData.MissileSpeed, allIntersections[0]);
                        }
