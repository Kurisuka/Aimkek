...
namespace Lil_BloodPump.SpellBlocking
{
    ...
    internal class EvadeTargetManager
    {
        ...
        public static void Attach(Menu mainMenu)
        {
            ...
            {
                foreach (var spell in Spells.Where(i => Lil_BloodPump.GameObjects.EnemyHeroes.Any(a => a.ChampionName == i.ChampionName)))
                {
        ...
        private static void OnUpdate()
        {
            ...
            foreach (var target in DetectedTargets.Where(i => ObjectManager.GetLocalPlayer().Distance(i.Obj.Position) < 700))
            {
                ...
                if (Lil_BloodPump.Vladimir.W.Ready && Menu["Brian.EvadeTargetMenu.EvadeTargetW"].Enabled 
                    && Lil_BloodPump.Vladimir.W.Cast(ObjectManager.GetLocalPlayer().ServerPosition.Extend(target.Start, 100)))
                {
                    return;
                }
            }
