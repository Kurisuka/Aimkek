...
namespace Lil_BloodPump.SpellBlocking
{
    ...
    public static class Geometry 
    {
        ...
        {
            return ObjectManager.GetLocalPlayer().Distance(anotherUnit, squared);
        }
        ...
        public class Polygon
        {
            ...
            public virtual void Draw(Color color, int width = 1)
            {
                for (var i = 0; i <= this.Points.Count - 1; i++)
                {
                    ...
                    Vector2 from;
                    Render.WorldToScreen(this.Points[i].To3D2(), out from);
                    Vector2 to;
                    Render.WorldToScreen(this.Points[nextIndex].To3D2(), out to);
                    Render.Line(from.X, from.Y, to.X, to.Y, color);
                }
            }
