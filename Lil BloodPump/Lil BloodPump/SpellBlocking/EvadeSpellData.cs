        ...
        public int DangerLevel
        {
            get
            {
                return EvadeManager.EvadeSpellMenu["DangerLevel_" + Slot] == null
                    ? _dangerLevel
                    : EvadeManager.EvadeSpellMenu["DangerLevel_" + Slot].As<MenuSlider>().Value;
            }
            set { _dangerLevel = value; }
        }

        public bool Enabled => EvadeManager.EvadeSpellMenu["Enabled" + Slot] == null || 
            EvadeManager.EvadeSpellMenu["Enabled" + Slot].As<MenuBool>().Enabled;

        public bool Tower => EvadeManager.EvadeSpellMenu["Tower" + Slot] == null ||
            EvadeManager.EvadeSpellMenu["Tower" + Slot].As<MenuBool>().Enabled;

        public bool IsReady()
        {
            return (CheckSpellName == "" || ObjectManager.GetLocalPlayer().SpellBook.GetSpell(Slot).Name == CheckSpellName) &&
                   ObjectManager.GetLocalPlayer().SpellBook.CanUseSpell(Slot);
        }
    }
}